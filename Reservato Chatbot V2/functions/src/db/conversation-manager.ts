...
export class ConversationManager {
    ...
    async getConversationHistory(sessionId: string, query: string) {

        const vectorStore = this.getVectorStore();
        const results = await vectorStore.similaritySearch(query, 20, { "sessionId": { "$eq": sessionId } });

        const sortedDocs = [...results];

        console.log("Pinecone results: " + JSON.stringify(results))
        ...
        const history: { role: "human" | "ai"; content: string }[] = [];
        ...
        for (const doc of sortedDocs) {
            ...
            if (isValidHuman && isValidAi) {
                history.push({ role: "human", content: human });
                history.push({ role: "ai", content: ai });
            }
        }
